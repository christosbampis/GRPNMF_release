...
%         % J{u,v} = filter2(G{u,v},I);
        ...
        temp=gradient(real(gaborArray{i,j}));
        temp2=gradient(temp);
        A=abs(conv2(img,temp,'same')).^2;
        B=conv2(img,real(gaborArray{i,j}),'same');
        C=conv2(img,temp2,'same');
%         
%         temp=gradient(real(gaborArray{i,j}));
%         temp2=gradient(temp);
%         A=abs(conv2(img,temp,'same')).^2;
...
%     for j=1:v
%         c=c+1;
%         bg(:,:,c)=(PSI{i,j}-mean(PSI{i,j}))/std(PSI{i,j},1);
%     end;
% end;
...
%     end
% end
% % title('Real parts of Gabor filters')
...
%     end
% end
% title('Magnitudes of Gabor filters')